...
int main( int argc, char** argv ) {
  ...
  while ( !feof(inp) ) {
    ...
    if ( nframes >= skip ) {
      ...
      ncalc++;                          // keep track of the number of s(k) that goes into the average
    }
  ...
  for ( int i=0 ; i<ntypes ; i++ ) {
    for ( int j=0 ; j<M ; j++ ) 
      avg_sk[i][j] *= 1.0 / double(ncalc);      // the actual averaging operation

    char nm[30];
    sprintf(nm, "sk%d.dat", i);
    write_kspace_data(nm, avg_sk[i]);               // write the results
  }

  cout << "Averaged over " << ncalc << " frames!" << endl;
